...
const FRAG = /* glsl */ `
  ...
  void main() {
    ...
    vec3 nightLights = texture2D(uNight, uv).rgb;
    float cloud = texture2D(uClouds, uv).a;
...
const ATMO_FRAG = /* glsl */ `
  ...
  void main() {
    ...
    vec3 col = vec3(0.30, 0.52, 1.0) * rim * lit * uBrightness * 0.5;
...
export async function createEarth(renderer) {
  ...
  const [day, night, spec, fallbackClouds] = await Promise.all([
    ...
    loadTexture(loader, FALLBACK_CLOUDS_URL, { srgb: false }),
  ]);
  const maxAniso = renderer.capabilities.getMaxAnisotropy();
  for (const t of [day, night, spec, fallbackClouds]) t.anisotropy = Math.min(8, maxAniso);

  const uniforms = {
    uDay: { value: day },
    uNight: { value: night },
    uSpec: { value: spec },
    uClouds: { value: fallbackClouds },
  ...
  async function refreshClouds() {
    ...
    try {
      ...
      live.anisotropy = Math.min(8, maxAniso);
      uniforms.uClouds.value = live;
      cloudStatus = 'live';
    } catch {
      cloudStatus = uniforms.uClouds.value === fallbackClouds ? 'bundled' : 'stale-live';
    }
  }
  refreshClouds();
  ...
  return {
    ...
    update(state) {
      if (Date.now() - lastCloudFetch > CLOUD_REFRESH_MS) refreshClouds();
